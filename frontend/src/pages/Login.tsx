...
export const Login = () => {
  ...
  // 1. AUTO-REDIRECT (Fixes Tab A when it wakes up)
  useEffect(() => {
    if (isAdmin) {
      navigate('/admin');
    }
  ...
  // 2. HANDLE MAGIC LINK (Fixes Tab C Flash)
  useEffect(() => {
    if (isMagicLink) {
  ...
  const handleOtpLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`Login link sent to ${email}. Click it to sign in!`);
    } catch (err: any) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  // --- RENDER: VERIFYING SCREEN (No "Flash") ---
  if (verifying) {
    return (
        ...
        <div className="p-8">
          {!isForgot && (
            <div className="flex border-b border-gray-200 mb-6">
              <button className={`flex-1 pb-2 text-sm font-semibold transition ${activeTab === 'password' ? 'border-b-2 border-black text-black' : 'text-gray-400'}`} onClick={() => setActiveTab('password')}>Password Login</button>
              <button className={`flex-1 pb-2 text-sm font-semibold transition ${activeTab === 'otp' ? 'border-b-2 border-black text-black' : 'text-gray-400'}`} onClick={() => setActiveTab('otp')}>Email Link (OTP)</button>
            </div>
          )}
          ...
          {isForgot ? (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <div className="relative"><Mail className="absolute left-3 top-3 text-gray-400 w-5 h-5" /><input type="email" required placeholder="Enter your email" className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-black" value={email} onChange={(e) => setEmail(e.target.value)} /></div>
            ...
            <>
              {activeTab === 'password' && (
                <form onSubmit={handlePasswordLogin} className="space-y-5">
                  <div className="relative"><Mail className="absolute left-3 top-3 text-gray-400 w-5 h-5" /><input type="email" required placeholder="Email Address" className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-black" value={email} onChange={(e) => setEmail(e.target.value)} /></div>
                  <div className="relative"><Lock className="absolute left-3 top-3 text-gray-400 w-5 h-5" /><input type={showPassword ? "text" : "password"} required placeholder="Password" className="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-black" value={password} onChange={(e) => setPassword(e.target.value)} /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-gray-400 hover:text-black">{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button></div>
